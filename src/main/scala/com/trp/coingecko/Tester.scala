...
//    println(client.getPrice(List("bitcoin","cardano"), List("usd"), includeMarketCap = true, include24hrVol = true, include24hrChange = true, includeLastUpdateAt = false))
//val coins = client.getPrice(List("bitcoin","cardano"), List("usd"), includeMarketCap = true, include24hrVol = true, include24hrChange = true, includeLastUpdateAt = false)


//      println(client.getSupportedVsCurrencies)
//    println(client.getCoinMarkets("usd"))
    println(client.getCoinMarkets("usd",
                                                List("bitcoin,cardano,ethereum,shiba-inu"),
                                                Some("market_cap_desc"),
                                                Some(10),
                                                Some(1),
                                                Some(false),
                                                Some("24h")))
